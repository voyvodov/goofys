...
const blockCacheReadAllSize = 20 * 1024 * 1024 // 20MiB
...
const maxPrefetchThreads = int32(16)
...
// BlockCache is cache for read-only files based on large blocks
...
func NewBlockCache(flags *FlagStorage) *BlockCache {
	...
	size = MaxInt(size,
		(blockCacheReadAllSize*2-1)/int(flags.BlockReadCacheSize)+1)
...
func (bc *BlockCache) acquirePrefetchThread() bool {
	...
	if num > maxPrefetchThreads {
		bc.releasePrefetchThread()
		assert(num <= maxPrefetchThreads+10,
...
func (bc *BlockCache) ensureBenchmark(fh *FileHandle) {
	...
	benchmarkOne := func(size int) (secs float64) {
		buf := make([]byte, size)
	...
	benchmark := func() {
		...
		latency := benchmarkOne(blockSize)
		throughput := blockCacheReadAllSize /
			benchmarkOne(blockCacheReadAllSize)

		maxNrBlock := blockCacheReadAllSize / blockSize

		nrBlock := 2
		bc.nrLatencyBlock = maxNrBlock
		for nrBlock < maxNrBlock {
			curLat := benchmarkOne(blockSize * nrBlock)
		...
		for nrBlock > bc.nrLatencyBlock {
			...
			curThrpt := float64(size) / benchmarkOne(size)
